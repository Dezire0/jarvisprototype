...
function appendMessagePreview(container, message) {
  ...
  meta.textContent = message.previewTitle || (message.previewSource === "assistant-browser" ? "Assistant browser preview" : "Desktop preview");
...
function startAssistantProgress(messageId, input) {
  ...
  state.liveTaskTimer = window.setInterval(() => {
    ...
    const nextIndex = currentSteps.length
      ? (Number(message.progressStepIndex) + 1) % currentSteps.length
      : 0;

    updateMessage(messageId, {
      progressSteps: currentSteps,
      progressStepIndex: nextIndex,
      progressLabel: currentSteps[nextIndex] || message.progressLabel || "작업을 진행하는 중"
    });
